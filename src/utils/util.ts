export function animation({
    ...
    if (!cb) throw 'callback not found.';

 
    const keys = easing.split('.');
    if (!Tween?.[keys[0]] || !Tween[keys[0]][keys[1]]) throw 'easing wrong.';
    ...
    const from = cur;
    const to = target;
    const during = Math.ceil(time / 17);

    const step = () => {
        const value = Tween[keys[0]][keys[1]](start, from, to - from, during);
        start++;
        if (start <= during) {
            cb && cb(value);
...
type animationFunc =(t:number, b:number, c:number, d:number,p?:number,a?:number,s?:number)=>number

type Article = {
    [args:string]:{
        [args:string]:animationFunc
    }

}
export function scrollTo(target:number, time = 400) {
    if (!window.requestAnimationFrame) {
        window.requestAnimationFrame = function (fn) {
           return setTimeout(fn, 17);
        };
    ...
    animation({ cur, target, cb: (v) => window.scrollTo(0, v) });
}
export const Tween:Article= {
    Linear: {
